...
for fileno=1:218
    ...
    while(line1~=-1)
        ...
        % flipped: I flipped this previously
        diffbarbsm=nrite2(fileno,no)-nleft2(fileno,no);
        if(abs(diffbarbsm)>1)
...
for i=1:length(jds)-1
    ...
    chir1_north(i,pos1)= chir1_yr(i,pos1);
    chir1_south(i,pos2)= chir1_yr(i,pos2);
    chir2_north(i,pos1)= chir2_yr(i,pos1);
    chir2_south(i,pos2)= chir2_yr(i,pos2);
end

% Calculating percentage of dextral and sinistral filaments in the northern
...
chirm3=chir1_yr(find(chir1_yr==0 & chir2_yr==1));
chirm4=chir2_yr(find(chir1_yr==1 & chir2_yr==0));
chirmn1= chir1_north(find(chir1_north~=0));
chirms1= chir1_south(find(chir1_south~=0));
chirmn2= chir2_north(find(chir2_north~=0));
chirms2= chir2_south(find(chir2_south~=0));
disp(sprintf('Dextral in Northern hemisphere (auto)= %f percent',100*length(find(chir1_north==1))/length(chirmn1)))
disp(sprintf('Sinistral in Northern hemisphere (auto)= %f percent',100*length(find(chir1_north==-1))/length(chirmn1)))
disp(sprintf('Dextral in Southern hemisphere (auto)= %f percent',100*length(find(chir1_south==1))/length(chirms1)))
disp(sprintf('Sinistral in Southern hemisphere (auto)= %f percent',100*length(find(chir1_south==-1))/length(chirms1)))
disp(sprintf('Dextral in Northern hemisphere (Man)= %f percent',100*length(find(chir2_north==1))/length(chirmn2)))
disp(sprintf('Sinistral in Northern hemisphere (Man)= %f percent',100*length(find(chir2_north==-1))/length(chirmn2)))
disp(sprintf('Dextral in Southern hemisphere (Man)= %f percent',100*length(find(chir2_south==1))/length(chirms2)))
disp(sprintf('Sinistral in Southern hemisphere (Man)= %f percent',100*length(find(chir2_south==-1))/length(chirms2)))
...
disp(sprintf('Filaments which changes 0 to 1= %f percent',100*length(find(chirm3))/length(chirm1)))
disp(sprintf('Filaments which changes 0 to 1= %f percent',100*length(find(chirm3))/length(chirm1)))
